 ...
 */
@RCommand(desc = "<zone|*> <on|off|inherit> - Sets a zones Suffocate Flag", name = "suffocate", usage = "<zone|*> <on|off|inherit>", minParam = 2, maxParam = 2)
    ...
    final void execute(Mod_Caller caller, String[] args) {
        ...
        try {
            ...
            if (user != null && !zone.permissionCheck(user, PermissionType.ENVIRONMENT)) {
                caller.sendError(RealmsTranslate.transformMessage("zone.perm.fail", "SUFFOCATE", zone.getName()));
                return;
            }
            ...
            caller.sendMessage(RealmsTranslate.transformMessage("zoneflag.set", "SUFFOCATE", (zone.getSuffocate() ? ChatColors.GREEN.concat("ON") : ChatColors.RED.concat("OFF")).concat(zone.getAbsoluteSuffocate().equals(ZoneFlag.INHERIT) ? ChatColors.PINK.concat("(INHERITED)") : ""), zone.getName()));

        }
        catch (InvaildZoneFlagException izfe) {
            caller.sendError(RealmsTranslate.transformMessage("zoneflag.invalid", "SUFFOCATE"));
        }
