 ...
 */
@RCommand(desc = "Sets a zones Potion Flag", name = "potion", usage = "<zone|*> <on|off|inherit>", minParam = 2, maxParam = 2)
final class SetPotionFlagCommand extends RealmsCommand{
    ...
    final void execute(Mod_Caller caller, String[] args){
        ...
        try {
            ...
            if (user != null && !zone.permissionCheck(user, PermissionType.COMBAT)) {
                caller.sendError("zone.perm.fail", "POTION", zone.getName());
                return;
            }
            ...
            caller.sendMessage("zoneflag.set", "POTION", (zone.getPotion() ? MCChatForm.GREEN.concat("ON") : MCChatForm.RED.concat("OFF")).concat(zone.getAbsolutePotion().equals(ZoneFlag.INHERIT) ? MCChatForm.PINK.concat("(INHERITED)") : ""), zone.getName());
        }
        catch (InvaildZoneFlagException ife) {
            caller.sendError("zoneflag.invalid", "POTION");
        }
