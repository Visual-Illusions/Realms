    ...
    final void execute(Mod_Caller caller, String[] args){
        ...
        try {
            ...
            if (user != null && !zone.permissionCheck(user, PermissionType.ENVIRONMENT)) {
                caller.sendError("zone.perm.fail", "PHYSICS", zone.getName());
                return;
            }
            ...
            caller.sendMessage("zoneflag.set", "PHYSICS", (zone.getPhysics() ? MCChatForm.GREEN.concat("ON") : MCChatForm.RED.concat("OFF")).concat(zone.getAbsolutePhysics().equals(ZoneFlag.INHERIT) ? MCChatForm.PINK.concat("(INHERITED)") : ""), zone.getName());
        }
        catch (InvaildZoneFlagException ife) {
            caller.sendError("zoneflag.invalid", "PHYSICS");
        }
