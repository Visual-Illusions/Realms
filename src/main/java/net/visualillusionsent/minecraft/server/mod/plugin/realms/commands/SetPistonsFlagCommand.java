 ...
 */
@RCommand(desc = "Sets a zones Pistons Flag", name = "pistons", usage = "<zone|*> <on|off|inherit>", minParam = 2, maxParam = 2)
final class SetPistonsFlagCommand extends RealmsCommand{
    ...
    final void execute(Mod_Caller caller, String[] args){
        ...
        try {
            ...
            if (user != null && !zone.permissionCheck(user, PermissionType.ENVIRONMENT)) {
                caller.sendError("zone.perm.fail", "PISTONS", zone.getName());
                return;
            }
            ...
            caller.sendMessage("zoneflag.set", "PISTONS", (zone.getPistons() ? MCChatForm.GREEN.concat("ON") : MCChatForm.RED.concat("OFF")).concat(zone.getAbsolutePistons().equals(ZoneFlag.INHERIT) ? MCChatForm.PINK.concat("(INHERITED)") : ""), zone.getName());
        }
        catch (InvaildZoneFlagException ife) {
            caller.sendError("zoneflag.invalid", "PISTONS");
        }
