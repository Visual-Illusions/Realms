 ...
 */
@RCommand(desc = "<zone|*> <on|off|inherit> - Sets a zones Fire Flag", name = "fire", usage = "<zone|*> <on|off|inherit>", minParam = 2, maxParam = 2)
final class SetFireFlagCommand extends RealmsCommand {
    ...
    final void execute(Mod_Caller caller, String[] args) {
        ...
        try {
            ...
            if (user != null && !zone.permissionCheck(user, PermissionType.ENVIRONMENT)) {
                caller.sendError(RealmsTranslate.transformMessage("zone.perm.fail", "FIRE", zone.getName()));
                return;
            }
            ...
            caller.sendMessage(RealmsTranslate.transformMessage("zoneflag.set", "FIRE", (zone.getFire() ? ChatColors.GREEN.concat("ON") : ChatColors.RED.concat("OFF")).concat(zone.getAbsoluteFire().equals(ZoneFlag.INHERIT) ? ChatColors.PINK.concat("(INHERITED)") : ""), zone.getName()));

        }
        catch (InvaildZoneFlagException ife) {
            caller.sendError(RealmsTranslate.transformMessage("zoneflag.invalid", "FIRE"));
        }
