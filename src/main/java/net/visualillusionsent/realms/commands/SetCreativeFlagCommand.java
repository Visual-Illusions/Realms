    ...
    final void execute(Mod_Caller caller, String[] args){
        ...
        try {
            ...
            if (user != null && !zone.permissionCheck(user, PermissionType.ENVIRONMENT)) {
                caller.sendError("zone.perm.fail", "CREATIVE", zone.getName());
                return;
            }
            ...
            if (zone.getAdventure()) {
                caller.sendError("creative.adventure.noset", "CREATIVE", "ADVENTURE");
                return;
            }
            zone.setCreative(theFlag);
            caller.sendMessage("zoneflag.set", "CREATIVE", (zone.getCreative() ? ChatFormat.GREEN.concat("ON") : ChatFormat.RED.concat("OFF")).concat(zone.getAbsoluteCreative().equals(ZoneFlag.INHERIT) ? ChatFormat.PINK.concat("(INHERITED)") : ""), zone.getName());
        }
        catch (InvaildZoneFlagException ife) {
            caller.sendError("zoneflag.invalid", "CREATIVE");
        }
