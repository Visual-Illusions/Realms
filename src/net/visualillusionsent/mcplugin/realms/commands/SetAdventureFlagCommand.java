 ...
 */
@RCommand(desc = "<zone|*> <on|off|inherit> - Sets a zones Adventure Flag", name = "adventure", usage = "<zone|*> <on|off|inherit>", minParam = 2, maxParam = 2)
    ...
    final void execute(Mod_Caller caller, String[] args) {
        ...
        try {
            ...
            if (user != null && !zone.permissionCheck(user, PermissionType.ENVIRONMENT)) {
                caller.sendError(RealmsTranslate.transformMessage("zone.perm.fail", "ADVENTURE", zone.getName()));
                return;
            }
            ...
            if (zone.getCreative()) {
                caller.sendError(RealmsTranslate.transformMessage("creative.adventure.noset", "ADVENTURE", "CREATIVE"));
                return;
            }

            zone.setAdventure(theFlag);
            caller.sendMessage(RealmsTranslate.transformMessage("zoneflag.set", "ADVENTURE", (zone.getAdventure() ? ChatColors.GREEN.concat("ON") : ChatColors.RED.concat("OFF")).concat(zone.getAbsoluteAdventure().equals(ZoneFlag.INHERIT) ? ChatColors.PINK.concat("(INHERITED)") : ""), zone.getName()));

        }
        catch (InvaildZoneFlagException ife) {
            caller.sendError(RealmsTranslate.transformMessage("zoneflag.invalid", "ADVENTURE"));
        }
