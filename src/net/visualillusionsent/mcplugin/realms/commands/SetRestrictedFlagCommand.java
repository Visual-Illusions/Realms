 ...
 */
@RCommand(desc = "<zone|*> <on|off|inherit> - Sets a zones Restricted Flag", name = "restricted", usage = "<zone|*> <on|off|inherit>", minParam = 2, maxParam = 2)
final class SetRestrictedFlagCommand extends RealmsCommand {
    ...
    final void execute(Mod_Caller caller, String[] args) {
        ...
        try {
            ...
            if (user != null && !zone.permissionCheck(user, PermissionType.ENVIRONMENT)) {
                caller.sendError(RealmsTranslate.transformMessage("zone.perm.fail", "RESTRICTED", zone.getName()));
                return;
            }
            ...
            caller.sendMessage(RealmsTranslate.transformMessage("zoneflag.set", "RESTRICTED", (zone.getRestricted() ? ChatColors.GREEN.concat("ON") : ChatColors.RED.concat("OFF")).concat(zone.getAbsoluteRestricted().equals(ZoneFlag.INHERIT) ? ChatColors.PINK.concat("(INHERITED)") : ""), zone.getName()));

        }
        catch (InvaildZoneFlagException ife) {
            caller.sendError(RealmsTranslate.transformMessage("zoneflag.invalid", "RESTRICTED"));
        }
