 ...
 */
@RCommand(desc = "Sets a zones Fall Flag", name = "fall", usage = "<zone|*> <on|off|inherit>", minParam = 2, maxParam = 2)
    ...
    final void execute(Mod_Caller caller, String[] args){
        ...
        try {
            ...
            if (user != null && !zone.permissionCheck(user, PermissionType.ENVIRONMENT)) {
                caller.sendError("zone.perm.fail", "FALL", zone.getName());
                return;
            }
            ...
            caller.sendMessage("zoneflag.set", "FALL", (zone.getFall() ? ChatFormat.GREEN.concat("ON") : ChatFormat.RED.concat("OFF")).concat(zone.getAbsoluteFall().equals(ZoneFlag.INHERIT) ? ChatFormat.PINK.concat("(INHERITED)") : ""), zone.getName());
        }
        catch (InvaildZoneFlagException ife) {
            caller.sendError("zoneflag.invalid", "FALL");
        }
