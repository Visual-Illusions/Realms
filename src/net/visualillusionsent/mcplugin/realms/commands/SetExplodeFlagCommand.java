 ...
 */
@RCommand(desc = "<zone|*> <on|off|inherit> - Sets a zones Explode Flag", name = "explode", usage = "<zone|*> <on|off|inherit>", minParam = 2, maxParam = 2)
    ...
    final void execute(Mod_Caller caller, String[] args) {
        ...
        try {
            ...
            if (user != null && !zone.permissionCheck(user, PermissionType.COMBAT)) {
                caller.sendError(RealmsTranslate.transformMessage("zone.perm.fail", "EXPLODE", zone.getName()));
                return;
            }
            ...
            caller.sendMessage(RealmsTranslate.transformMessage("zoneflag.set", "EXPLODE", (zone.getExplode() ? ChatColors.GREEN.concat("ON") : ChatColors.RED.concat("OFF")).concat(zone.getAbsoluteExplode().equals(ZoneFlag.INHERIT) ? ChatColors.PINK.concat("(INHERITED)") : ""), zone.getName()));

        }
        catch (InvaildZoneFlagException ife) {
            caller.sendError(RealmsTranslate.transformMessage("zoneflag.invalid", "EXPLODE"));
        }
