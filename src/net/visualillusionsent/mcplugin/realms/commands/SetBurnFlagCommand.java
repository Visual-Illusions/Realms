 ...
 */
@RCommand(desc = "<zone|*> <on|off|inherit> - Sets a zones Burn Flag", name = "burn", usage = "<zone|*> <on|off|inherit>", minParam = 2, maxParam = 2)
final class SetBurnFlagCommand extends RealmsCommand {
    ...
    final void execute(Mod_Caller caller, String[] args) {
        ...
        try {
            ...
            if (user != null && !zone.permissionCheck(user, PermissionType.ENVIRONMENT)) {
                caller.sendError(RealmsTranslate.transformMessage("zone.perm.fail", "BURN", zone.getName()));
                return;
            }
            ...
            zone.setBurn(theFlag);
            caller.sendMessage(RealmsTranslate.transformMessage("zoneflag.set", "BURN", (zone.getBurn() ? ChatColors.GREEN.concat("ON") : ChatColors.RED.concat("OFF")).concat(zone.getAbsoluteBurn().equals(ZoneFlag.INHERIT) ? ChatColors.PINK.concat("(INHERITED)") : ""), zone.getName()));

        }
        catch (InvaildZoneFlagException ife) {
            caller.sendError(RealmsTranslate.transformMessage("zoneflag.invalid", "BURN"));
        }
