 ...
 */
@RCommand(desc = "Sets a zones Starve Flag", name = "starve", usage = "<zone|*> <on|off|inherit>", minParam = 2, maxParam = 2)
final class SetStarveFlagCommand extends RealmsCommand{
    ...
    final void execute(Mod_Caller caller, String[] args){
        ...
        try {
            ...
            if (user != null && !zone.permissionCheck(user, PermissionType.ENVIRONMENT)) {
                caller.sendError("zone.perm.fail", "STARVE", zone.getName());
                return;
            }
            ...
            caller.sendError("zoneflag.set", "STARVE", (zone.getStarve() ? MCChatForm.GREEN.concat("ON") : MCChatForm.RED.concat("OFF")).concat(zone.getAbsoluteStarve().equals(ZoneFlag.INHERIT) ? MCChatForm.PINK.concat("(INHERITED)") : ""), zone.getName());
        }
        catch (InvaildZoneFlagException ife) {
            caller.sendError("zoneflag.invalid", "STARVE");
        }
