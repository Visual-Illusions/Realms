 ...
 */
@RCommand(desc = "<zone|*> <on|off|inherit> - Sets a zones Dispensers Flag", name = "dispensers", usage = "<zone|*> <on|off|inherit>", minParam = 2, maxParam = 2)
final class SetDispenseFlagCommand extends RealmsCommand {
    ...
    final void execute(Mod_Caller caller, String[] args) {
        ...
        try {
            ...
            if (user != null && !zone.permissionCheck(user, PermissionType.ENVIRONMENT)) {
                caller.sendError(RealmsTranslate.transformMessage("zone.perm.fail", "DISPENSERS", zone.getName()));
                return;
            }
            ...
            zone.setDispensers(theFlag);
            caller.sendMessage(RealmsTranslate.transformMessage("zoneflag.set", "DISPENSERS", (zone.getDispensers() ? ChatColors.GREEN.concat("ON") : ChatColors.RED.concat("OFF")).concat(zone.getAbsoluteDispensers().equals(ZoneFlag.INHERIT) ? ChatColors.PINK.concat("(INHERITED)") : ""), zone.getName()));

        }
        catch (InvaildZoneFlagException ife) {
            caller.sendError(RealmsTranslate.transformMessage("zoneflag.invalid", "DISPENSERS"));
        }
